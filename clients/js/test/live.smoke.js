...
async function main() {
  const client = createClient({ baseUrl: BASE, application: "sdk-smoke" });
  ...
  const auto = await client.chat({ messages: "Reply with exactly: pong", model: "auto", maxTokens: 200 });
  ...
  const pin = await client.chat({ messages: "Reply with exactly: pong", model: pinModel, maxTokens: 200 });
  ...
  const model = asLangChainModel(client, { workflow: "smoke", maxTokens: 200 });
