...
`classifiedBy`, and token `usage`. Every call is logged to MongoDB as a **RequestRecord**.
...
- **Usage logging** — one `RequestRecord` per call, recording **both the model requested
  and the model served** (so realised savings are measurable). Unknown-model calls log
  `totalCost: 0`; add a registry entry to get accurate billing.
...
- **Controlled routing** — human rules first; then the automated mode a human enabled:
  ...
  task→model map, with AI per-call task classification). Plus response caching for exact
  duplicates and provider fallback.
...
modelRequested, taskType, promptTokens, completionTokens, totalTokens, inputCost,
outputCost, totalCost, latencyMs, status, routingDecision, cacheHit, knownPricing`
